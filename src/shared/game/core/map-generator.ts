...
import { noise2D, normalize } from "../utils"
...
export function generateMap(cfg: MapConfig): TileType[][] {
    ...
    // Температурный градиент (север=холод юг=жара)
    for (let y = 0; y < H; y++)
        for (let x = 0; x < W; x++)
    ...
    for (let y = 0; y < H; y++) {
        for (let x = 0; x < W; x++) {
            if (elev[y][x] < oceanRatio) { map[y][x] = TileType.OCEAN; continue }

            const t = temp[y][x], m = mois[y][x]
...
function waterZones(map: TileType[][], W: number, H: number): void {
    ...
    for (let y = 0; y < H; y++)
        for (let x = 0; x < W; x++)
            if (!isW(map[y][x])) { dist[y][x] = 0; q.push(x, y) }

    let head = 0
    while (head < q.length) {
        const cx = q[head++], cy = q[head++], nd = dist[cy][cx] + 1
